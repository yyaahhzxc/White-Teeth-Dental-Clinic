const sqlite3 = require('sqlite                // Show current users with their roles
                db.all("SELECT id, username, employeeRole, userRole FROM users ORDER BY id", (err, employees) => {
                    if (err) {
                    ...
                    employees.forEach(emp => {
                        console.log(`ID: ${emp.id}, Username: ${emp.username}, Employee Role: "${emp.employeeRole}", User Role: "${emp.userRole}"`);
                    });
                    console.log(`Total: ${employees.length} users\n`);;
    ...
    return new Promise((resolve, reject) => {
        db.serialize(() => {
            ...
            db.all("SELECT DISTINCT employeeRole FROM users WHERE employeeRole IS NOT NULL AND employeeRole != '' ORDER BY employeeRole", (err, rows) => {
                ...
                db.all("SELECT userId, username, employeeRole, userRole FROM users ORDER BY userId", (err, employees) => {
                    if (err) {
                    ...
                    console.log('=== CURRENT USERS ===');
                    employees.forEach(emp => {
                        console.log(`ID: ${emp.userId}, Username: ${emp.username}, Employee Role: "${emp.employeeRole}", User Role: "${emp.userRole}"`);
                    });
                    console.log(`Total: ${employees.length} users\n`);
